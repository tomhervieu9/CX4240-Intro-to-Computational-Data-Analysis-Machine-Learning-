...
for p in range(0,pno):
    ...
    # logistic regression;
    iterno = 200
    theta = np.zeros((dimno, 1))
    eta = np.power(0.1, 8)
    for i in range(0, iterno):
        print '--iteration %d of %d\n' % (i + 1,iterno)
        grad = eta * ( \
               trainx.dot((trainy - 1).T) + \
               trainx.dot( (np.exp(-theta.T.dot(trainx)).T) * 1.0 \
               / (1 + np.exp(-theta.T.dot(trainx)).T) ) \
               )  
        theta = theta + grad
    ...
    for i in range(0, testno):
        ...
        pytest[i, 1] = 1.0 / (1 + np.exp(-theta.T.dot(testx[:, i])))
        pytest[i, 0] = 1 - pytest[i, 1]
        
        if pytest[i, 0] > pytest[i, 1]:
            predy[i, 0] = 0
        else:
            predy[i, 0] = 1
        ...
        img_class[int(predy[i, 0]), :, i] = testx[:, i]
    
    accuracy[0, p] = count_acc * 1.0 / testno
    print accuracy[0,p]
